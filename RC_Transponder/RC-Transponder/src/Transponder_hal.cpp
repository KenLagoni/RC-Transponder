...
//	this->_GPS = new GPSL80Lite();
	...
	ResetGPS();
	this->_GPS.begin(this->_serialGPS, GPS_SERIAL_BAUDRATE, this->_serialAUX);
	...
	this->_radio.begin(chipSelectPin,resetPin,busyPin,dio1Pin,0,0,txEnPin,rxEnPin, led2Pin);
...
void Transponder_hal::setBrownOut(void)
{
	...
	/* Configure the brown-out detector so that the
	   ...
	   voltage */
	SYSCTRL->BOD33.reg = (
	/* This sets the brown-out voltage (BOD-) level to 2.72-2.82v.
	   And the BOD+ hysteresis to 2.84-2.92v
		See datasheet table 37-21. Page 880 */
